...
sudo docker run -d --name anovos_demo anovos-demo:latest
sudo docker exec -it anovos_demo /bin/bash -c "python --version && spark-submit --version && java -version"
sudo docker stop anovos_demo
