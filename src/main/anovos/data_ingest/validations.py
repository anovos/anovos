    ...
    def wrapper(*args, **kwargs):
        ...
        if "list_of_cols" in all_kwargs.keys():
            ...
            if any(x not in idf.columns for x in list_of_cols):
                raise TypeError(
                    f"Invalid input for column(s) in the function {func.__name__}. Column(s) not found in the dataframe: {set(list_of_cols) - set(idf.columns)}."
                )
            if len(list_of_cols) != len(set(list_of_cols)):
                raise TypeError(
                    f"Duplicate input for column(s) in the function {func.__name__}."
                )
        ...
        if "file_type" in all_kwargs.keys():
            if all_kwargs.get("file_type") not in ("csv", "parquet", "avro", "json"):
                raise TypeError(
                    f"Invalid input for file_type in the function {func.__name__}. file_type should be csv, parquet, avro, json - Received '{all_kwargs.get('file_type')}'."
                )
        ...
        if func.__name__ == "write_dataset":
            ...
            if any(x not in idf.columns for x in column_order):
                raise TypeError(
                    f"Invalid input for column_order in the function {func.__name__}. Column(s) not found in the dataframe: {set(column_order) - set(idf.columns)}."
                )
        ...
        if func.__name__ == "concatenate_dataset":
            if all_kwargs.get("method_type") not in ("index", "name"):
                raise TypeError(
                    f"Invalid input for method_type in the function {func.__name__}. method_type should be index or name - Received '{all_kwargs.get('method_type')}'."
                )

        if func.__name__ == "join_dataset":
            if all_kwargs.get("join_type") not in (
                "inner",
                "full",
                "left",
                "right",
                ...
                raise TypeError(
                    f"Invalid input for join_type in the function {func.__name__}. join_type should be index or name - Received '{all_kwargs.get('join_type')}'."
                )
            ...
            for idf in idfs:
                if any(x not in idf.columns for x in join_cols):
                    raise TypeError(
                        f"Invalid input for join column(s) in the function {func.__name__}. Column(s) not found in the dataframe(s): {set(join_cols) - set(idf.columns)}."
                    )
        ...
        if func.__name__ == "rename_column":
            ...
            if len(list_of_newcols) != len(list_of_cols):
                raise TypeError(
                    f"Mismatch between number of columns in list_of_cols ({len(list_of_cols)}) and list_of_newcols ({len(list_of_newcols)}) in the function {func.__name__}."
                )
        ...
        if func.__name__ == "recast_column":
            ...
            if len(list_of_dtypes) != len(list_of_cols):
                raise TypeError(
                    f"Mismatch between number of columns in list_of_cols ({len(list_of_cols)}) and number of datatypes ({len(list_of_dtypes)}) in list_of_dtypes in the function {func.__name__}."
                )
