...
def timestamp_to_unix (spark, idf, list_of_cols, precision="s", tz='local', output_mode="replace"):
    """
    ...
    localtz = spark.sql("SET spark.sql.session.timeZone")\
                        .select('value').rdd.flatMap(lambda x:x).collect()[0]
    ...
    for i in list_of_cols:
        ...
        odf = odf.withColumn(modify_col[output_mode], (F.col(i+"_local").cast("double")*factor[precision]).cast("long"))\
                 .drop(i+"_local")
    return odf


def unix_to_timestamp (spark, idf, list_of_cols, precision="s", tz='local', output_mode="replace"):
    """
    :param spark: Spark Session
    :param idf: Input Dataframe
    :param list_of_cols: List of columns to transform e.g., ["col1","col2"].
    ...
    for i in list_of_cols:
        ...
        odf = odf.withColumn(modify_col[output_mode], F.to_timestamp(F.col(i)/ factor[precision]))
        if (tz in ('gmt','utc')) & (localtz.lower() not in ('gmt','utc')):
            odf = odf.withColumn(modify_col[output_mode], F.to_utc_timestamp(modify_col[output_mode],localtz))
        
    return odf


def timezone_conversion(spark, idf, list_of_cols, given_tz, output_tz, output_mode="replace"):
    """
    :param spark: Spark Session
    :param idf: Input Dataframe
    :param list_of_cols: List of columns to transform e.g., ["col1","col2"].
    ...
    for i in list_of_cols:
        ...
        odf = odf.withColumn(modify_col[output_mode], F.from_utc_timestamp(F.to_utc_timestamp(i,given_tz),output_tz))
...
def dateformat_conversion(idf, list_of_cols, input_format="%Y-%m-%d %H:%M:%S", 
                          output_format="%Y-%m-%d %H:%M:%S", output_mode="replace"):
    """
    :param spark: Spark Session
    :param idf: Input Dataframe
    :param list_of_cols: List of columns to transform e.g., ["col1","col2"].
...
def time_diff(idf, ts1, ts2, unit, output_mode="append"):
    ...
    :return: Output Dataframe with derived column
    """
    argument_checker('time_diff', {'list_of_cols': [ts1,ts2], 'all_columns': idf.columns, 'output_mode': output_mode})
    
    factor_mapping = {'second': 1,  'minute': 60, 'hour': 3600, 'day': 86400, 
                       'week': 604800, 'month': 2628000, 'year': 31536000}
    ...
    odf = idf.withColumn(ts1+'_'+ ts2+'_'+ unit + 'diff', F.abs((F.col(ts1).cast("double") - F.col(ts2).cast("double")))/factor)
...
def time_elapsed(idf, list_of_cols, unit, output_mode="append"):
    ...
    odf = idf
    for i in list_of_cols:
        odf = odf.withColumn(i + '_'+ unit + 'diff', F.abs((F.lit(F.current_timestamp()).cast("double") - F.col(i).cast("double")))/factor)
...
def adding_timeUnits(idf, list_of_cols, unit, unit_value, output_mode="append"):
    ...
    odf = idf
    for i in list_of_cols:
        odf = odf.withColumn(i + '_adjusted', F.col(i) + F.expr('Interval ' + str(unit_value) + ' ' + unit))
...
def timestamp_comparison(idf, list_of_cols, comparison_type, comparison_value, 
                         comparison_format="%Y-%m-%d %H:%M:%S", output_mode="append"):
    """
    :param spark: Spark Session
    :param idf: Input Dataframe
    :param list_of_cols: List of columns to transform e.g., ["col1","col2"].
    ...
    for i in list_of_cols:
        if comparison_type == "greater_than":
            odf = odf.withColumn(i+'_compared', F.when(F.col(i) > F.lit(base_ts),1).otherwise(0))
...
def is_monthStart(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_ismonthStart', F.when(F.to_date(F.col(i)) == F.col(i+'_monthStart'), 1).otherwise(0))\
                .drop(i+'_monthStart')
...
def is_monthEnd(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_ismonthEnd', F.when(F.to_date(F.col(i)) == F.col(i+'_monthEnd'), 1).otherwise(0))\
                .drop(i+'_monthEnd')
...
def is_yearStart(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isyearStart', F.when(F.to_date(F.col(i)) == F.col(i+'_yearStart'), 1).otherwise(0))\
                .drop(i+'_yearStart')
...
def end_of_year(idf, list_of_cols, output_mode="append"):
    ...
    odf = idf
    for i in list_of_cols:
        odf = odf.withColumn(i+'_yearEnd', F.concat_ws("-", F.year(i), F.lit(12), F.lit(31)).cast("date"))
...
def is_yearEnd(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isyearEnd', F.when(F.to_date(F.col(i)) == F.col(i+'_yearEnd'), 1).otherwise(0))\
                .drop(i+'_yearEnd')
...
def is_quarterStart(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isquarterStart', F.when(F.to_date(F.col(i)) == F.col(i+'_quarterStart'), 1).otherwise(0))\
...
def end_of_quarter(idf, list_of_cols, output_mode="append"):
    ...
    odf = idf
    for i in list_of_cols:
        odf = odf.withColumn(i+'_quarterEnd', F.to_date(F.date_trunc("quarter", i)) + 
                             F.expr('Interval 3 months') + F.expr('Interval -1 day'))
...
def is_quarterEnd(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isquarterEnd', F.when(F.to_date(F.col(i)) == F.col(i+'_quarterEnd'), 1).otherwise(0))\
                .drop(i+'_quarterEnd')
...
def is_yearFirstHalf(idf, list_of_cols, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isFirstHalf', F.when(F.month(F.col(i)).isin(*range(1, 7)), 1).otherwise(0))
...
def is_selectedHour(idf, list_of_cols, start_hour, end_hour, output_mode="append"):
    ...
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isselectedHour', F.when(F.hour(F.col(i)).isin(*list_of_hrs), 1).otherwise(0))
...
def is_weekend(idf, list_of_cols, output_mode="append"):
    ...
    odf = idf
    for i in list_of_cols:
        odf = odf.withColumn(i+'_isweekend', F.when(F.dayofweek(F.col(i)).isin([1,7]), 1).otherwise(0))
...
def window_aggregator(idf, list_of_cols, list_of_aggs, order_col, window_type='expanding', 
    ...
    odf = idf
    window_upper = Window.unboundedPreceding if window_type == 'expanding' else -int(window_size)
    if partition_col:
        window = Window.partitionBy(partition_col).orderBy(order_col).rowsBetween(window_upper, 0)
    else:
        window = Window.partitionBy().orderBy(order_col).rowsBetween(window_upper, 0)
    
    def agg_funcs(col):
        mapping = {'count': F.count(col).over(window).alias(col+'_count'),
                   ...
                   'median': F.expr('percentile_approx('+col+', 0.5)').over(window).alias(col+'_median')}
        derived_cols = []
...
def lagged_ts(idf, list_of_cols, lag, output_type='ts', tsdiff_unit='days', partition_col='', output_mode='append'):
    """
    :param spark: Spark Session
    :param idf: Input Dataframe
    :param list_of_cols: List of columns to transform e.g., ["col1","col2"].
    ...
    for i in list_of_cols:
        ...
        if output_type == 'ts_diff':
            odf = time_diff(odf, i, i+"_lag"+str(lag), unit=tsdiff_unit, output_mode="append")
