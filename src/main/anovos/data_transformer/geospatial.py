from .geo_utils import (EARTH_RADIUS, from_latlon_decimal_degrees, to_latlon_decimal_degrees, 
                       haversine_distance, vincenty_distance, euclidean_distance)
from pyspark.sql import functions as F
from pyspark.sql import types as T


def geo_format_latlon(idf, list_of_lat, list_of_lon, input_format, output_format, result_prefix=None, 
                      geohash_precision=8, radius=EARTH_RADIUS, output_mode="append"):
    """
    ...
    if len(result_prefix) != len(list_of_lat):
        raise TypeError("result_prefix must have the same length as list_of_lat and list_of_lon")

    f_to_latlon_dd = F.udf(lambda loc: to_latlon_decimal_degrees(loc, input_format, radius), 
                            T.ArrayType(T.FloatType()))

    from_latlon_dd_ = lambda loc: from_latlon_decimal_degrees(loc, output_format, radius, geohash_precision)
    if output_format in ["dd", "radian", "cartesian"]:
        ...
        f_from_latlon_dd = F.udf(from_latlon_dd_, T.ArrayType(T.ArrayType(T.FloatType())))
    ...
    for i, (lat, lon) in enumerate(zip(list_of_lat, list_of_lon)):
        ...
        odf = odf.withColumn(col+"_temp", F.array(lat, lon))\
                 .withColumn(col+"_"+output_format, f_from_latlon_dd(f_to_latlon_dd(col+"_temp")))
    
        if output_format in ["dd", "dms", "radian"]:
            odf = odf.withColumn(col+"_lat_"+output_format, F.col(col+"_"+output_format)[0])\
                     .withColumn(col+"_lon_"+output_format, F.col(col+"_"+output_format)[1])\
                     .drop(col+"_"+output_format)

        if output_format == "cartesian":
            odf = odf.withColumn(col+"_x", F.col(col+"_"+output_format)[0])\
...
def geo_format_cartesian(idf, list_of_x, list_of_y, list_of_z, output_format, result_prefix=None, 
                         geohash_precision=8, radius=EARTH_RADIUS, output_mode="append"):
    ...
    if len(result_prefix) != len(list_of_x):
        raise TypeError("result_prefix must have the same length as list_of_x, list_of_y and list_of_y")

    f_to_latlon_dd = F.udf(lambda loc: to_latlon_decimal_degrees(loc, "cartesian", radius), 
                            T.ArrayType(T.FloatType()))
    ...
    for i, (x, y, z) in enumerate(zip(list_of_x, list_of_y, list_of_z)):
        ...
        odf = odf.withColumn(col+"_temp", F.array(x, y, z))\
                 .withColumn(col+"_"+output_format, f_from_latlon_dd(f_to_latlon_dd(col+"_temp")))
...
def geo_format_geohash(idf, list_of_geohash, output_format, result_prefix=None, 
                      geohash_precision=8, radius=EARTH_RADIUS, output_mode="append"):
    ...
    f_to_latlon_dd = F.udf(lambda loc: to_latlon_decimal_degrees(loc, "geohash", radius), 
                            T.ArrayType(T.FloatType()))
    ...
    for i, geohash in enumerate(list_of_geohash):
        ...
        odf = odf.withColumn(col+"_"+output_format, f_from_latlon_dd(f_to_latlon_dd(geohash)))
...
def location_distance(idf, list_of_cols_loc1, list_of_cols_loc2, loc_format="dd", result_prefix=None,
                      distance_type="haversine", unit="m", radius=EARTH_RADIUS, vincenty_model="WGS-84", output_mode="append"):
    ...
    if loc_format != format_required:
        ...
        if format_required == "dd":
            loc1, loc2 = ["temp_loc1_lat_dd", "temp_loc1_lon_dd"], ["temp_loc2_lat_dd", "temp_loc2_lon_dd"]

        elif format_required == "radian":
            loc1, loc2 = ["temp_loc1_lat_radian", "temp_loc1_lon_radian"], ["temp_loc2_lat_radian", "temp_loc2_lon_radian"]
        
        elif format_required == "cartesian":
            loc1, loc2 = ["temp_loc1_x", "temp_loc1_y", "temp_loc1_z"], ["temp_loc2_x", "temp_loc2_y", "temp_loc2_z"]
        
        idf = idf.withColumn('temp_loc1', F.array(*loc1))\
                 .withColumn('temp_loc2', F.array(*loc2))\
                 .drop(*(loc1+loc2))
    else:
        idf = idf.withColumn('temp_loc1', F.array(*list_of_cols_loc1))\
                 .withColumn('temp_loc2', F.array(*list_of_cols_loc2))

    if distance_type == "vincenty":
        compute_distance = lambda x1, x2: vincenty_distance(x1, x2, unit, vincenty_model)
    elif distance_type == "haversine":
        compute_distance = lambda x1, x2: haversine_distance(x1, x2, "radian", unit, radius)
    else:
        compute_distance = lambda x1, x2: euclidean_distance(x1, x2)
    
    f_compute_distance = F.udf(compute_distance, T.FloatType())

    col_prefix = result_prefix if result_prefix is not None else "_".join(list_of_cols_loc1) + "_" + "_".join(list_of_cols_loc2)
    odf = idf.withColumn(col_prefix + "_" + "distance", f_compute_distance('temp_loc1', 'temp_loc2'))\
