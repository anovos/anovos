...
from anovos.shared.utils import attributeType_segregation, ends_with, flatten_dataframe, transpose_dataframe
from anovos.data_analyzer.stats_generator import measures_of_percentiles
from anovos.data_transformer.datetime import argument_checker, timeUnits_extraction,unix_to_timestamp, lagged_ts, dateformat_conversion,string_to_timestamp
...
#     match emails and urls to avoid returning chunks of them
    ...
    HEADERS = ['PATH','FILE_ID','REGEX_LABEL','LINE_NUM','MATCH','START','END','MATCH_LEN','NORM_LINE_LEN','CONTEXT','PARSED',]
    ...
    if idf.select(col).dtypes[0][1] == "timestamp" or idf.select(col).dtypes[0][1] == "date":
        ...
        output_df = unix_to_timestamp(spark, idf, col, precision = precision, tz=tz,output_mode=output_mode).orderBy(id_col,col)
        ...
        for line_num, line in enumerate(file_lines):
            ...
            for match_info in regex_text(line):
                ye,mo,da,ho,mi,se = match_info["PARSED"].year,\
            ...
            bl.append([match_info['CONTEXT'],datetime.datetime(bl_int[0],bl_int[1],bl_int[2],bl_int[3],bl_int[4],bl_int[5])])
...
def ts_eligiblity_check(spark,idf,col,id_col,opt=1):
    ...
    lagged_df = lagged_ts(ts_processed_feats(spark,idf,col,id_col).select("yyyymmdd_col").distinct().orderBy("yyyymmdd_col"), "yyyymmdd_col", lag = 1, tsdiff_unit= "days", output_mode= "append").orderBy("yyyymmdd_col")

    diff_lagged_df = list(np.around(lagged_df.withColumn("daydiff",F.datediff("yyyymmdd_col","yyyymmdd_col_lag1"))\
                              .where(F.col("daydiff").isNotNull())\
                              .groupBy().agg(F.mean("daydiff").alias("mean"),F.variance("daydiff").alias("variance"),F.stddev("daydiff").alias("stdev"))\
                              .withColumn("coef_of_var_lag",F.col("stdev")/F.col("mean"))\
                              .rdd.flatMap(lambda x: x).collect(),3))
    
    p1 = measures_of_percentiles(spark, ts_processed_feats(spark,idf,col,id_col).groupBy(id_col).agg(F.countDistinct("yyyymmdd_col").alias("id_date_pair")), list_of_cols="id_date_pair")
    p2 = measures_of_percentiles(spark, ts_processed_feats(spark,idf,col,id_col).groupBy("yyyymmdd_col").agg(F.countDistinct(id_col).alias("date_id_pair")), list_of_cols="date_id_pair")
...
def ts_viz_data(spark,idf,x_col,y_col,id_col,precision="s",tz="local",output_mode="append",output_type="daily",n_cat=10):
    ...
    if y_col_dtype == "string":
        ...
        top_cat = list(idf.groupBy(y_col).count().orderBy("count",ascending=False).limit(int(n_cat)).select(y_col).toPandas()[y_col].values)
        idf = idf.withColumn(y_col,F.when(F.col(y_col).isin(top_cat),F.col(y_col)).otherwise(F.lit("Others")))
        idf = ts_processed_feats(spark,idf,x_col,id_col)

        if output_type == "daily":

            odf = idf.groupBy(y_col,"yyyymmdd_col").agg(F.count(y_col).alias('count')).orderBy("yyyymmdd_col").withColumnRenamed("yyyymmdd_col",x_col).toPandas()
            
        elif output_type == "hourly":

            odf = idf.groupBy(y_col,"daypart_cat").agg(F.count(y_col).alias('count')).orderBy("daypart_cat").toPandas()
            
        elif output_type == "weekly":

            odf = idf.groupBy(y_col,"dow").agg(F.count(y_col).alias('count')).orderBy("dow").toPandas()
        ...
        if output_type == "daily":
            ...
            odf = idf.groupBy("yyyymmdd_col").agg(F.min(y_col).alias("min"),F.max(y_col).alias("max"),F.mean(y_col).alias("mean"),\
                                   F.expr('percentile(' + y_col + ', array(0.5))')[0].alias('median')).orderBy("yyyymmdd_col").withColumnRenamed("yyyymmdd_col",x_col).toPandas()
            
        elif output_type == "hourly":
            
            odf = idf.groupBy("daypart_cat").agg(F.min(y_col).alias("min"),F.max(y_col).alias("max"),F.mean(y_col).alias("mean"),\
                                   F.expr('percentile(' + y_col + ', array(0.5))')[0].alias('median')).orderBy("daypart_cat").toPandas()
            

        elif output_type == "weekly":
            
            odf = idf.groupBy("dow").agg(F.min(y_col).alias("min"),F.max(y_col).alias("max"),F.mean(y_col).alias("mean"),\
                                   F.expr('percentile(' + y_col + ', array(0.5))')[0].alias('median')).orderBy("dow").toPandas()
...
def ts_loop_cols_pre(idf,id_col):
    ...
    ts_col = []
    for i in idf.dtypes:
        if ((i[1] in ["string","object"]) or (i[1] in ["long","bigint"] and (check_val_ind(idf.select(F.max(F.length(i[0]))).rdd.flatMap(lambda x: x).collect()[0])>9) and (idf.select(i[0]).distinct()))) and i[0]!= id_col:
            lc.append(i[0])
        else:
            pass
...
def ts_preprocess(spark, idf, id_col,output_path,run_type="local"):
        ...
        for j in [num_cols,cat_cols]:
            for k in j:
                for l in ["daily","hourly","weekly"]:
                    print(i,j,k,l)
                    f = ts_viz_data(spark,idf,i,k,id_col=id_col,precision="ms",tz="local",output_mode="append",output_type=l,n_cat=10)
                    f.to_csv(ends_with(local_path) + i + "_" + k + "_" + l + ".csv",index=False)
    ...
    for i in ts_loop_cols_post:
        for j in range(1,3):
            ...
            f.to_csv(ends_with(local_path) + "stats_" + str(i) + "_" + str(j)+".csv",index=False)
    ...
    f = pd.DataFrame([[",".join(idf.columns)],[",".join(c1)],[",".join(c2)],[",".join(c3)],[",".join(c4)],[",".join(num_cols)],[",".join(cat_cols)]], columns = ['cols'])
