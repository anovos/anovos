    ...
    def wrapper(*args, **kwargs):
        ...
        if "treatment_threshold" in all_kwargs:
            ...
            if (treatment_threshold < 0) | (treatment_threshold > 1):
                raise TypeError(
                    f"Invalid input for Treatment Threshold Value in the function {func.__name__}. treatment_threshold should be between 0 & 1 - Received '{treatment_threshold}'."
                )
        ...
        if "output_mode" in all_kwargs.keys():
            if all_kwargs.get("output_mode") not in ("replace", "append"):
                raise TypeError(
                    f"Invalid input for output_mode in the function {func.__name__}. output_mode should replace or append - Received '{all_kwargs.get('output_mode')}'."
                )

        if func.__name__ in ("nullColumns_detection", "invalidEntries_detection"):
            if all_kwargs.get("treatment_method") not in (
                "row_removal",
                "MMM",
                "null_replacement",
                "column_removal",
            ...
            if treatment_threshold:
                ...
                all_kwargs["treatment_configs"][
                    "treatment_threshold"
                ] = treatment_threshold
            else:
                if all_kwargs.get("treatment_method") == "column_removal":
                    raise TypeError(
                        f"Missing input for Treatment Threshold Value in the function {func.__name__}. treatment_threshold should be between 0 & 1."
                    )

        if func.__name__ == "outlier_detection":
            if all_kwargs.get("detection_side") not in ("upper", "lower", "both"):
                raise TypeError(
                    f"Invalid input for detection_side in the function {func.__name__}. detection_side should be upper, lower or both - Received '{all_kwargs.get('detection_side')}'."
                )
            if all_kwargs.get("treatment_method") not in (
                "null_replacement",
                "row_removal",
                ...
                raise TypeError(
                    f"Invalid input for treatment_method in the function {func.__name__}. treatment_method should be null_replacement, row_removal or value_replacement - Received '{all_kwargs.get('treatment_method')}'."
                )
