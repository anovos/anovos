...
def check_precomputed_scores(stats, idfs):
    if isinstance(stats, dict):
        if any([x not in list(stats.keys()) for x in ["mean", "stddev", "kurtosis"]]):
            raise ValueError(
                f"Invalid input for stats. Keys of stats must contain 'mean', 'stddev', 'kurtosis'."
            )
    value_len = -1
    for value in list(stats.values()):
        if value_len > 0:
            if len(value) != value_len:
                raise ValueError(
                    f"Invalid input for stats. Length of all values of stats should be the same."
                )
        else:
            value_len = len(value)

        for config in value:
            if any([x not in list(config.keys()) for x in ["file_path", "file_type"]]):
                raise ValueError(
                    f"Invalid input for values of stats. The config for each pre-computed statistics result \
