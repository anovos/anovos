...
def charts_to_objects(idf, list_of_cols='all', drop_cols=[], label_col=None, event_label=1,
                      ...
                      drift_detector=False, source_path="NA", output_path=''):
    global num_cols
    global cat_cols
    import timeit
    ...
    for idx, col in enumerate(list_of_cols):
        ...
        if col in cat_cols:
            ...
            f.write_json(ends_with(output_path) + "freqDist_" + col)

            if label_col:
                f = plot_eventRate(idf_encoded,col,label_col,event_label,cutoffs_path)
                f.write_json(ends_with(output_path) + "eventRist_" + col)
            ...
            if drift_detector:
                ...
                f.write_json(ends_with(output_path) + "drift_" + col)
        
        if col in num_cols:
            f = plot_outlier(idf,col,split_var=None)
            f.write_json(ends_with(output_path) + "outlier_" + col)
