...
def line_chart_gen_stability(df1, df2, col):
    ...
    return dp.Group(dp.Text("#"), dp.Text(f5), dp.Plot(f1), dp.Group(dp.Plot(f2), dp.Plot(f3), dp.Plot(f4), columns=3),
                    rows=4, label=col)
...
df_si = df_si_[
    ["attribute", "stability_index", "mean_si", "stddev_si", "kurtosis_si", "mean_cv", "stddev_cv", "kurtosis_cv",
     "flagged"]]
...
data = [['Descriptive Statistics', 'Global Summary', '',
         'Summarize global information about the datasets like number of rows and columns, name and number of categorical and numerical attributes'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measures of Shape',
         'Describe the distribution(or pattern) of different attributes in the datasets using skewness and kurtosis'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measures of Central Tendency',
         'Describe the central position of each attributes in datasets by finding basic measures like mean, median and mode'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measure Of Percentiles',
         'Indicate the value below which a given percentage of data of given attribute falls'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measures Of Dispersion',
         'Measure the spread of data about the mean e.g., Standard Deviation, Variance, Covariance, IQR and range of each attribute of datasets'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measures Of Cardinality',
         'Measure the count of unique values present in each attribute'],
        ['Descriptive Statistics', 'Statistics by Metric Type', 'Measures Of Counts',
         'Measure the sparsity of the datasets, e.g., fill count and percenatge, missing value count and percentage and nonzero count and percenatge'],
        ['Descriptive Statistics', 'Attribute Visualization', 'Numerical',
         'Visualize the distributions of Numerical attributes using Histograms'],
        ['Descriptive Statistics', 'Attribute Visualization', 'Categorical',
         'Visualize the distributions of Categorical attributes using Barplot'],
        ['Quality check', 'Column Level', 'IDness Detection', ''],
        ['Quality check', 'Column Level', 'Null Detection',
         'Detect the sparsity of the datasets, e.g., count and percentage of missing value of attributes'],
        ['Quality check', 'Column Level', 'Baisedness Detection',
         'Detect the baisedness of the attributes by finding the mode and its percenatge(value that is most frequent in the data)'],
        ['Quality check', 'Column Level', 'InvalidEntries Detection',
         'Detect the entries and count of invalid values or noise present in the datasets'],
        ['Quality check', 'Column Level', 'Outlier Detection',
         'Used to detect and visualize the outlier present in numerical attributes of the datasets'],
        ['Quality check', 'Row Level', 'Duplicate Detection',
         'Measure number of rows in the datasets that have same value for each attribute'],
        ['Quality check', 'Row Level', 'Rows WMissingFeats',
         'Measure the count/percentage of rows which have missing/null attributes'],
        ['Attribute Associations', 'Association Matrix', 'Correlation Matrix',
         'Measure the strength of relationship among each attribute by finding correlation coefficient having range -1.0 to 1.0.'],
        ['Attribute Associations', 'Association Matrix', 'IV Calculations',
         'Information Value Calculations- Used to rank variables on the basis of their importance.Greater the value of IV higher rthe attribute importance. IV less than 0.02 is not useful for prediction'],
        ['Attribute Associations', 'Association Matrix', 'IG Calculations',
         'Information Gain- Measures the reduction in entropy by splitting a dataset according to given value of a attribute.'],
        ['Attribute Associations', 'Association Matrix', 'Varibale clustering',
         'Divides the numerical attributes into disjoint or hierarchical clusters based on linear relationship of attributes'],
        ['Attribute Associations', '', '', ''],
        ['Attribute Associations', 'Association Plot', 'Correlation Matrix',
         'Used to Visualize the strength of relationship among attributes by ploting heat map'],
        ['Attribute Associations', 'Association Plot', 'IV Calculations',
         'Used to Visualize attribute importance of in increasing or decreasing order using barplot'],
        ['Attribute Associations', 'Association Plot', 'IG Calculations',
         'Used to visualize the purity of attributes in dataset using barplot i.e., how a change to the datasets impact the distribution of data.'],
        ['Attribute Associations', 'Association Plot', 'Varibale clustering',
         'Used to visualize how numerical featrures are group together into different clusters formed by variable clustering'],
        ['Attribute Associations', 'Attribute to Target Association', 'Numerical ',
         'Used to Visualize the bucket-wise percentage/ditribution of data for Numerical attributes having Target Variable greater/less than or equal to given threshold value'],
        ['Attribute Associations', 'Attribute to Target Association', 'Categorical',
         'Used to Visualize the Category-wise percentage/distributions of data for categorical attributes having Target Varibale greater/less than or equal to  given threshold value.'],
        ['Data Drift & Data Stability', 'Data Drift Analysis', 'PSI',
         'Population Stability Index- Measure how much a attribute has shifted in distribution between two sample of dataset(target and source datasets )'],
        ['Data Drift & Data Stability', 'Data Drift Analysis', 'JSD',
         'Jensen-Shannon Divergence- Used to quantify the difference(or similarity) between distributions of two sample data(target and source datasets). It ranges between 0-1. smaller the score of JSD higher the similarity of two datasets. '],
        ['Data Drift & Data Stability', 'Data Drift Analysis', 'HD',
         'Hellinger Distance- Measure the similarity in distribution between two sample of dataset(target and source datasets ). Smaller the value of hellinger distance higher the similarity in distribution of two sample dataset.'],
        ['Data Drift & Data Stability', 'Data Drift Analysis', 'KS',
         'kolmogorov-Smirnov Test- It quantifies a distance between distribution of two sample dataset to check the similarity between them. Greater the value of K-S test p-value higher the similarity between two dataset.']]
        ...
        if tab_name == "quality_checker":
            if i == "duplicate_detection":
                ...
                unique_rows_count = " No. Of Unique Rows : " + str(
                    int(duplicate_recs[duplicate_recs["metric"] == "unique_rows_count"].value.values))
                rows_count = " No. of Rows : " + str(
                    int(duplicate_recs[duplicate_recs["metric"] == "rows_count"].value.values))
                df_list.append([dp.Text("### " + str(remove_u_score(i))),
                                dp.Group(dp.Text(unique_rows_count), dp.Text(rows_count), rows=2)])

            else:
                df_list.append([dp.Text("### " + str(remove_u_score(i))), dp.DataTable(
                    pd.read_csv(ends_with(p2) + str(tab_name) + "_" + str(i) + ".csv").round(3))])
            ...
            for j in list_tabs_all[idx]:
                ...
                if j == "correlation_matrix":
                    ...
                    df_list_ = pd.read_csv(ends_with(p2) + str(tab_name) + "_" + str(j) + ".csv").round(3).sort_values(
            ...
            df_list.append(dp.DataTable(pd.read_csv(ends_with(p2) + str(tab_name) + "_" + str(i) + ".csv").round(3),
                                        label=remove_u_score(list_tabs_all[idx][index])))

    return df_list


data = [['Descriptor Statistics', 'Global Summary', '',
         'Summarize global information about the datasets like number of rows and columns, name and number of categorical and numerical attributes'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measures of Shape',
         'Describe the distribution(or pattern) of different attributes in the datasets using skewness and kurtosis'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measures of Central Tendency',
         'Describe the central position of each attributes in datasets by finding basic measures like mean, median and mode'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measure Of Percentiles',
         'Indicate the value below which a given percentage of data of given attribute falls'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measures Of Dispersion',
         'Measure the spread of data about the mean e.g., Standard Deviation, Variance, Covariance, IQR and range of each attribute of datasets'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measures Of Cardinality',
         'Measure the count of unique values present in each attribute'],
        ['Descriptor Statistics', 'Statistics by Metric Type', 'Measures Of Counts',
         'Measure the sparsity of the datasets, e.g., fill count and percenatge, missing value count and percentage and nonzero count and percenatge'],
        ['Descriptor Statistics', 'Attribute Visualization', 'Numerical',
         'Visualize the distributions of Numerical attributes using Histograms'],
        ['Descriptor Statistics', 'Attribute Visualization', 'Categorical',
         'Visualize the distributions of Categorical attributes using Barplot'],
        ['Quality check', 'Column Level', 'IDness Detection', ''],
        ['Quality check', 'Column Level', 'Null Detection',
         'Detect the sparsity of the datasets, e.g., count and percentage of missing value of attributes'],
        ...
        ['Quality check', 'Row Level', 'Rows WMissingFeats',
         'Measure the count/percentage of rows which have missing/null attributes'],
        ['Association & Interactions', 'Association Matrix', 'Correlation Matrix',
         'Measure the strength of relationship among each attribute by finding correlation coefficient having range -1.0 to 1.0.'],
        ['Association & Interactions', 'Association Matrix', 'IV Calculations',
         'Information Value Calculations- Used to rank variables on the basis of their importance.Greater the value of IV higher rthe attribute importance. IV less than 0.02 is not useful for prediction'],
        ['Association & Interactions', 'Association Matrix', 'IG Calculations',
         'Information Gain- Measures the reduction in entropy by splitting a dataset according to given value of a attribute.'],
        ['Association & Interactions', 'Association Matrix', 'Varibale clustering',
         'Divides the numerical attributes into disjoint or hierarchical clusters based on linear relationship of attributes'],
        ['Association & Interactions', '', '', ''],
        ['Association & Interactions', 'Association Plot', 'Correlation Matrix',
         'Used to Visualize the strength of relationship among attributes by ploting heat map'],
        ['Association & Interactions', 'Association Plot', 'IV Calculations',
         'Used to Visualize attribute importance of in increasing or decreasing order using barplot'],
        ['Association & Interactions', 'Association Plot', 'IG Calculations',
         'Used to visualize the purity of attributes in dataset using barplot i.e., how a change to the datasets impact the distribution of data.'],
        ['Association & Interactions', 'Association Plot', 'Varibale clustering',
         'Used to visualize how numerical featrures are group together into different clusters formed by variable clustering'],
        ['Association & Interactions', 'Attribute to Target Association', 'Numerical ',
         'Used to Visualize the bucket-wise percentage/ditribution of data for Numerical attributes having Target Variable greater/less than or equal to given threshold value'],
        ['Association & Interactions', 'Attribute to Target Association', 'Categorical',
         'Used to Visualize the Category-wise percentage/distributions of data for categorical attributes having Target Varibale greater/less than or equal to  given threshold value.'],
        ['Data Drift & Data Stability', 'Data Drift Analyzer', 'PSI',
         'Population Stability Index- Measure how much a attribute has shifted in distribution between two sample of dataset(target and source datasets )'],
        ['Data Drift & Data Stability', 'Data Drift Analyzer', 'JSD',
         'Jensen-Shannon Divergence- Used to quantify the difference(or similarity) between distributions of two sample data(target and source datasets). It ranges between 0-1. smaller the score of JSD higher the similarity of two datasets. '],
        ['Data Drift & Data Stability', 'Data Drift Analyzer', 'HD',
         'Hellinger Distance- Measure the similarity in distribution between two sample of dataset(target and source datasets ). Smaller the value of hellinger distance higher the similarity in distribution of two sample dataset.'],
        ['Data Drift & Data Stability', 'Data Drift Analyzer', 'KS',
         'kolmogorov-Smirnov Test- It quantifies a distance between distribution of two sample dataset to check the similarity between them. Greater the value of K-S test p-value higher the similarity between two dataset.']]
...
def main(base_path):
    ...
    def drift_text_gen(drifted_feats, len_feats, t2):
        if drifted_feats == 0:
            ...
            if int(t2 * len_feats) < drifted_feats:
                text = "*The above barometer indicates that " + str(drifted_feats) + " out of " + str(
                    len_feats) + " (" + str(np.round((100 * drifted_feats / len_feats),
                                                     2)) + "%) attributes were found to be deviating from its base data behaviour. Further, it has computed the overall data health based on data drift across the attributes of interest as well. It can be inferred that based on the safe netting threshold of " + str(
                    t2 * 100) + "% chosen (approx " + str(
                    int(t2 * len_feats)) + ") we could see it to be deviated by " + str(
                    drifted_feats - int(t2 * len_feats)) + " attributes which contributes to (-)" + str(
                    np.round(100 * (drifted_feats - int(t2 * len_feats)) / (len_feats),
                             2)) + "% below the mark. Please refer to the metric values as displayed in the above table & comparison plot for better understanding*"
            else:
                text = "*The above barometer indicates that " + str(drifted_feats) + " out of " + str(
                    len_feats) + " (" + str(np.round((100 * drifted_feats / len_feats),
                                                     2)) + "%) attributes were found to be deviating from its base data behaviour. Further, it has computed the overall data health based on data drift across the attributes of interest as well. It can be inferred that based on the safe netting threshold of " + str(
                    t2 * 100) + "% chosen (approx " + str(int(
                    t2 * len_feats)) + ") we could not see any attribute to be deviated below the mark. Please refer to the metric values as displayed in the above table & comparison plot for better understanding*"
        else:
            text = ""
        return text
    ...
    for i in all_charts_num_1:
        ...
        try:
            col_name = \
            open(ends_with(p1) + i).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0].split("''")[0][
            1:]
    ...
    for j in all_charts_num_2:
        ...
        try:
            col_name = \
            open(ends_with(p1) + j).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0].split("''")[0][
            1:]
    ...
    for k in all_charts_num_3:
        try:
            if bool(islabel):
                col_name = \
                open(ends_with(p1) + k).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[2].split(">")[
                    1]
                all_charts_num_3_.append(dp.Plot(go.Figure(json.load(open(ends_with(p1) + k))), label=col_name))
            else:
                col_name = open(ends_with(p1) + k).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0][
                           1:]
                all_charts_num_3_.append(dp.Plot(go.Figure(json.load(open(ends_with(p1) + k))), label=col_name))
        except:
            pass
    ...
    for l in all_charts_cat_1:
        ...
        col_name = \
        open(ends_with(p1) + l).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0].split("''")[0][1:]
        if col_name == label:
    ...
    for m in all_charts_cat_2:
        ...
        try:
            col_name = \
            open(ends_with(p1) + m).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0].split("''")[0][
            1:]
    ...
    if bool(islabel):
        l1 = dp.Group(dp.Text("### Attribute to Target Association"), dp.Text(
            "*Bivariate Distribution considering the event captured across different attribute splits (or categories)*"))
        l2 = dp.Group(dp.Select(
            blocks=[dp.Group(dp.Select(blocks=all_charts_num_2_, type=dp.SelectType.DROPDOWN), label="Numeric"),
    ...
    a1 = "Overall data contains **" + str(rows_count) + "** records across **" + str(
        numcols_count + catcols_count) + "** attributes of which **" + str(
        numcols_count) + "** are continuous while **" + str(catcols_count) + "** are categorical."
    if label is None:
        ...
        for l in all_charts_cat_1:
            ...
            col_name = \
            open(ends_with(p1) + l).readlines()[0].split("hovertemplate")[1].split(":")[1].split("=")[0].split("''")[0][
            1:]
            ...
            if col_name == label:
                ...
                label_fig_ = go.Figure(data=[go.Pie(labels=x_val, values=y_val, textinfo='label+percent',
                                                    insidetextorientation='radial', pull=[0, 0.1],
    ...
    try:
        x1 = list(
            pd.read_csv(ends_with(p2) + "stats_generator_measures_of_dispersion.csv").query("`cov`>1").attribute.values)
        if len(x1) > 0:
    ...
    try:
        x2 = list(
            pd.read_csv(ends_with(p2) + "stats_generator_measures_of_shape.csv").query("`skewness`>0").attribute.values)
        if len(x2) > 0:
    ...
    try:
        x4 = list(
            pd.read_csv(ends_with(p2) + "stats_generator_measures_of_shape.csv").query("`kurtosis`>3").attribute.values)
        if len(x4) > 0:
    ...
    try:
        x6 = list(pd.read_csv(ends_with(p2) + "stats_generator_measures_of_counts.csv").query(
            "`fill_pct`<0.7").attribute.values)
        if len(x6) > 0:
    ...
    try:
        x7 = list(pd.read_csv(ends_with(p2) + "quality_checker_biasedness_detection.csv").query(
            "`flagged`>0").attribute.values)
        if len(x7) > 0:
    ...
    try:
        x10 = list(pd.read_csv(ends_with(p2) + "/association_evaluator_IV_calculation.csv").query(
    ...
    x1 = x1[["Outliers", "Significant Attributes", "Positive Skewness", "Negative Skewness", "High Variance",
    ...
    if bool(data_drift_check):
        dp.Report(dp.HTML(
            '<html><img src="https://plsadaptive.s3.amazonaws.com/eco/images/partners/Y5Zm9VJkHtunAAdmazwsO2lhTwftfYNfk5aP4RJ7.png" style="height:40px;display:flex;margin:auto;float:right"></img></html>'), \
                  dp.Text("# ML-Anovos Report"), \
                  dp.Select(blocks=[
                      dp.Group(dp.Text("# "), dp.Text("1 . " + a1), dp.Text("2 . " + a2), label_fig_,
                               dp.Text("3 . Attributes Diagnosis View"), dp.DataTable(x1), dp.Text("4 . " + a5), \
                               dp.Group(dp.BigNumber(heading="Attributes Drifted",
                                                     value=str(str(drifted_feats) + " out of " + str(len_feats))), \
                                        dp.BigNumber(heading="% Drifted",
                                                     value=str(np.round((100 * drifted_feats / len_feats), 2)) + "%",
                                                     change=str(np.round(
                                                         100 * (drifted_feats - int(t2 * len_feats)) / (len_feats),
                                                         2)) + "%", is_upward_change=flag_var), \
                                        dp.BigNumber(heading="Attributes Impacted by Instability",
                                                     value=str(len(stability_attr)) + " out of " + str(
                                                         len(total_stability_attr))), \
                                        dp.BigNumber(heading="% Attributes which are Unstable", value=str(
                                            np.round(100 * len(stability_attr) / len(total_stability_attr), 2)) + "%"),
                                        columns=4), \
                               dp.Text("# "), \
                               dp.Text("# "), label="Executive Summary"), \
                      dp.Group(dp.Select(blocks=[ \
                          dp.Group(dp.Group(dp.Text("## "), dp.Text("### Data Dictionary"), dp.DataTable(data_dict)),
                                   label="Data Dictionary"), \
                          dp.Group(dp.Text("##"), dp.Text("### Metric Definitions"), dp.DataTable(metric_dict),
                                   label="Metric Dictionary")], type=dp.SelectType.TABS), label="Wiki"), \
                      dp.Group(
                          dp.Text("# "), \
                          dp.Text(
                              "*Descriptive Statistics summarizes the basic information about the data elements and their individual distribution*"), \
                          dp.Text("### Global Summary"), \
                          dp.Text(
                              "*This section details about the dimension of dataset and the details of attributes across respective data type*"), \
                          dp.Group(dp.Text(" Total Number of Records : **" + str(rows_count) + "**"),
                                   dp.Text(" Total Number of Attributes : **" + str(columns_count) + "**"),
                                   dp.Text(" Number of Continuous Attributes : **" + str(numcols_count) + "**"),
                                   dp.Text(" Continuous Attributes Name : **" + str(numcols_name) + "**"),
                                   dp.Text(" Number of Categorical Attributes : **" + str(catcols_count) + "**"),
                                   dp.Text(" Categorical Attributes Name : **" + str(catcols_name) + "**"), rows=6), \
                          dp.Text("### Statistics by Metric Type"), \
                          dp.Text(
                              "*Gives an overall representation of the data anatomy as measured across the different statistical tests*"), \
                          dp.Select(blocks=data_analyzer_output(p2, "stats_generator"), \
                                    type=dp.SelectType.TABS), \
                          dp.Text("# "), \
                          dp.Text("### Attribute Visualization"), \
                          dp.Text(
                              "*Univariate representation of attributes can be studied here through the histogram (continuous) / bar plots (categorical). For a comprehensive view, a restriction is made to the number of buckets based on the user input*"), \
                          dp.Group(dp.Select(blocks= \
                                                 [dp.Group(
                      ...
                      dp.Group(dp.Select(blocks=[
                          dp.Group(
                              ...
                              dp.Group(data_analyzer_output(p2, "quality_checker")[1][0],
                                       data_analyzer_output(p2, "quality_checker")[1][1],
                                       dp.Text("*Distribution of Continuous Attributes using Density curves*"),
                                       dp.Select(blocks=all_charts_num_3_, type=dp.SelectType.DROPDOWN), rows=4), \
                      ...
                      dp.Group(dp.Text("# "), \
                               ...
                               dp.Select(blocks=data_analyzer_output(p2, tab_name="association_evaluator"),
                      ...
                      dp.Group(dp.Text("# "), \
                               dp.Text(
                                   "*Useful in capturing the underlying data drift / deviation of the attribute composition as compared to the source data used for analysis*"), \
                               dp.Text("### Data Drift Analysis"), \
                               dp.DataTable(drift_df), \
                               dp.Text("*Flagging of Data Drift basis the threshold of " + str(
                               ...
                               dp.Group(
                                   dp.Text("*Representation of Attributes across different computed Drift Metrics*"),
                                   dp.Text(drift_text_gen(drifted_feats, len_feats, t2)), columns=2), \
                               dp.Text("## "), \
                               dp.Text("## "), \
                               dp.Text("### Data Stability Analysis"), \
                               dp.DataTable(df_si), \
                               dp.Text(
                                   "*Stability Index is computed by analyzing the coefficient of variation of statistical metrics such as mean, kurtosis, and standard deviation across " + str(
                                       n_df_stability) + " datasets*"), \
                               dp.Select(blocks=line_chart_list, type=dp.SelectType.DROPDOWN), \
                               dp.Text(
                                   "**{ Interpretation of Stability Index >> 0-1: Very Unstable, 1-2: Unstable, 2-3: Marginally Stable, 3-3.5: Stable, 3.5-4: Very Stable }**"), \
                               dp.Text("## "), \
                               dp.Text("## "), \
