...
def drift_statistics(idf_target, idf_source, list_of_cols='all', drop_cols=[], method_type='PSI',
...
def stabilityIndex_computation(*idfs, list_of_cols='all', drop_cols=[],
    ...
    odf = odf.replace(np.nan, None).withColumn('mean_si', f_score_cv(F.col('mean_cv'))) \
        ...
        .withColumn('flagged', F.when(F.lit(threshold).isNull(), "-") \
                    .otherwise(
        F.when((F.col('stability_index') < threshold) | (F.col('stability_index').isNull()), 1).otherwise(0)))
