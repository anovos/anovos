...
def test_write_dataset(spark_session: SparkSession):
    ...
    write_dataset(df, sample_output_path, "parquet", {'mode': 'overwrite'}, [])
    assert os.path.isfile(sample_output_path + "_SUCCESS")
    column_order1 = ['ifa', 'age', 'workclass', 'fnlwgt', 'logfnl', 'education', 'education-num', 'marital-status',
                     'income', 'occupation', 'relationship', 'race', 'sex', 'capital-gain', 'capital-loss',
                     'hours-per-week', 'native-country']
    write_dataset(df, file_path=sample_output_path, file_type="parquet", file_configs={'mode': 'overwrite'},
