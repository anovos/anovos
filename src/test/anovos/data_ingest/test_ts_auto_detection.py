...
def test_regex_date_time_parser(spark_session):
    ...
    odf=regex_date_time_parser(spark_session,idf=df,id_col="STATE",col="YR",tz="local",val_unique_cat=4,trans_cat="int_c",save_output=None, output_mode="append")
    odf_pd=odf.toPandas()
    len(odf.columns) == 11
    odf.select("YR_ts").dtypes[0][1] == 'date'
    odf_pd['YR_ts'][0] == datetime.date(1970, 1, 1)

    odf=regex_date_time_parser(spark_session,idf=df,id_col="STATE",col="YR",tz="local",val_unique_cat=4,trans_cat="int_c",save_output="unit_testing/output/timeseries/regex_date_time_parser/")
    assert os.path.isfile("unit_testing/output/timeseries/regex_date_time_parser/" + "_SUCCESS")

#ts_preprocess
def test_ts_preprocess(spark_session):
    df = read_dataset(spark_session, sample_csv, "csv",file_configs = {"header": "True", "delimiter": "," ,"inferSchema": "True"})
    odf = ts_preprocess(spark_session, idf=df, id_col='STATE', output_path="unit_testing/output/timeseries/ts_preprocess/", tz_offset="local", run_type="local")
