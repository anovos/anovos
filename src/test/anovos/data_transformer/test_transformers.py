...
def test_z_standardization(spark_session):
    ...
    odf = z_standardization(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], model_path= "unit_testing/models/")
    ...
    try:
        odf=z_standardization(spark_session, df, list_of_cols=["education-num"], pre_existing_model=True, model_path="unit_testing/models/")
    ...
    odf = z_standardization(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], output_mode="append")
    assert len(odf.columns) == 20

def test_IQR_standardization(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = IQR_standardization(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    ...
    try:
        odf = IQR_standardization(spark_session, df, list_of_cols=["education-num"], pre_existing_model=True, model_path= "unit_testing/models/")
    except Exception as error:
        assert str(error)=="list index out of range"
    ...
    odf = IQR_standardization(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], output_mode="append")
    assert len(odf.columns) == 20

def test_normalization(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = normalization(df, list_of_cols=["age","fnlwgt","hours-per-week"], model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    ...
    try:
        odf = normalization(df, list_of_cols=["age","fnlwgt","hours-per-week"], pre_existing_model=True, model_path= "unit_testing/models/")
    except Exception as error:
        assert str(error)=="list index out of range"    
    ...
    odf = normalization(df, list_of_cols=["age","fnlwgt","hours-per-week"], output_mode="append")
    assert len(odf.columns) == 20

#imputation
def test_imputation_sklearn(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = imputation_sklearn(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], method_type="KNN", model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
...
def test_imputation_MMM(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = imputation_MMM(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week","relationship","race"], method_type="mode", model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    assert odf.where(F.col("age").isNull()).count() == 0
    assert odf.where(F.col("fnlwgt").isNull()).count() == 0
    assert odf.where(F.col("hours-per-week").isNull()).count() == 0
...
def test_auto_imputation(spark_session):
    ...
    assert len(odf[0].columns) == 18
    assert odf[0].where(F.col("age").isNull()).count() == 0
    assert odf[0].where(F.col("fnlwgt").isNull()).count() == 0
...
def test_PCA_latentFeatures(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = PCA_latentFeatures(spark_session, df, list_of_cols=["age","fnlwgt","logfnl","education-num","hours-per-week"],explained_variance_cutoff=0.3, model_path= "unit_testing/models/")
    assert len(odf.columns) < len(df.columns)
    assert len(odf.columns) == 13

    try:
        odf = PCA_latentFeatures(spark_session, df, list_of_cols=["education-num"],explained_variance_cutoff=0.3, pre_existing_model=True, model_path= "unit_testing/models/")
    except Exception as error:
        assert str(error)=="list index out of range"     

    odf = PCA_latentFeatures(spark_session, df, list_of_cols=[],explained_variance_cutoff=0.3)
    assert len(odf.columns) == len(df.columns)
    assert len(odf.columns) == 17
...
def test_autoencoder_latentFeatures(spark_session):
    ...
    odf = autoencoder_latentFeatures(spark_session, df, list_of_cols=[] ,epochs=20, reduction_params=0.5)
    assert len(odf.columns) == len(df.columns)
    assert len(odf.columns) == 17
...
def test_feature_transformation(spark_session):
    ...
    odf = feature_transformation(df,list_of_cols=["age","fnlwgt","hours-per-week"], output_mode = "append")
    assert len(odf.columns) == 20

def test_boxcox_transformation(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = boxcox_transformation(df, list_of_cols=["age","fnlwgt","hours-per-week"], boxcox_lambda=0.5)
    assert len(odf.columns) == 17
    odf_pd=odf.where(F.col("ifa") == "27520a").toPandas()
    assert approx(odf_pd["age"][0])==7.14142842854285
    assert approx(odf_pd["fnlwgt"][0])==399.6936326738268
    ...
    odf = boxcox_transformation(df, list_of_cols=["age","fnlwgt","hours-per-week"], boxcox_lambda=0.5, output_mode = "append")
    assert len(odf.columns) == 20

def test_outlier_categories(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf= outlier_categories(spark_session, df, list_of_cols=["workclass","education","relationship","race","native-country"],  max_category=12, model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    ...
    try:
        odf= outlier_categories(spark_session, df, list_of_cols=["occupation"],  max_category=12, pre_existing_model=True, model_path= "unit_testing/models/")
    except Exception as error:
        assert str(error)=="list index out of range"    

    odf= outlier_categories(spark_session, df, list_of_cols=[],  max_category=12)
    assert odf.select("workclass").distinct().count() == df.select("workclass").distinct().count()
    assert odf.select("education").distinct().count() == df.select("education").distinct().count()
    assert odf.select("relationship").distinct().count() == df.select("relationship").distinct().count()
    assert odf.select("native-country").distinct().count() == df.select("native-country").distinct().count()
    assert odf.select("race").distinct().count() == df.select("race").distinct().count()
    assert odf.select("occupation").distinct().count() == df.select("occupation").distinct().count()
    assert odf.select("sex").distinct().count() == df.select("sex").distinct().count()
    assert odf.select("marital-status").distinct().count() == df.select("marital-status").distinct().count()  

    odf= outlier_categories(spark_session, df, list_of_cols=["workclass","education","relationship","race","native-country"],  max_category=12, output_mode = "append")
    assert len(odf.columns) == 22

#binning
def test_attribute_binning(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = attribute_binning(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], bin_size=20, model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    odf_min_dict = odf.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    odf_max_dict = odf.describe().where(F.col("summary") == "max").toPandas().to_dict('list')
    ...
    try:
        odf = attribute_binning(spark_session, df, list_of_cols=["education-num"], bin_size=20, pre_existing_model=True, model_path= "unit_testing/models/")
    except Exception as error:
        assert str(error)=="list index out of range"    
    ...
    odf_min_dict = odf.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    odf_max_dict = odf.describe().where(F.col("summary") == "max").toPandas().to_dict('list')
    df_min_dict = df.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    df_max_dict = df.describe().where(F.col("summary") == "max").toPandas().to_dict('list')    
    assert round(float(odf_min_dict['age'][0])) == round(float(df_min_dict['age'][0]))
    assert round(float(odf_min_dict['fnlwgt'][0])) == round(float(df_min_dict['fnlwgt'][0]))
    assert round(float(odf_max_dict['age'][0])) == round(float(df_max_dict['age'][0])) 
    assert round(float(odf_max_dict['fnlwgt'][0])) == round(float(df_max_dict['fnlwgt'][0]))

    odf = attribute_binning(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], bin_size=20, output_mode="append")
    assert len(odf.columns) == 20 


def test_monotonic_binning(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf = monotonic_binning(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], label_col="income", event_label="<=50K", bin_method="equal_range", bin_size=10)
    assert len(odf.columns) == 17
    odf_min_dict = odf.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    odf_max_dict = odf.describe().where(F.col("summary") == "max").toPandas().to_dict('list')
    assert round(float(odf_min_dict['age'][0])) == 1
    ...
    odf = monotonic_binning(spark_session, df, list_of_cols=[], label_col="income", event_label="<=50K", bin_method="equal_range", bin_size=10)
    odf_min_dict = odf.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    odf_max_dict = odf.describe().where(F.col("summary") == "max").toPandas().to_dict('list')
    df_min_dict = df.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    df_max_dict = df.describe().where(F.col("summary") == "max").toPandas().to_dict('list')    
    ...
    odf = monotonic_binning(spark_session, df, list_of_cols=["age","fnlwgt","hours-per-week"], label_col="income", event_label="<=50K", bin_method="equal_range", bin_size=10, output_mode = "append")
    assert len(odf.columns) == 20

#categorical_to_numerical_transformation   
def test_cat_to_num_unsupervised(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf =cat_to_num_unsupervised(spark_session, df, list_of_cols=["workclass","relationship","marital-status"], drop_cols=["ifa"], method_type=1, index_order="frequencyDesc", cardinality_threshold=100, model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    odf_min_dict = odf.describe().where(F.col("summary") == "min").toPandas().to_dict('list')
    ...
    odf =cat_to_num_unsupervised(spark_session, df, list_of_cols=[], drop_cols=["ifa"], method_type=1, index_order="frequencyDesc", cardinality_threshold=100)
    ...
    odf =cat_to_num_unsupervised(spark_session, df, list_of_cols=["workclass","relationship","marital-status"], drop_cols=["ifa"], method_type=1, index_order="frequencyDesc", cardinality_threshold=100, output_mode="append")
    assert len(odf.columns) == 20
...
def test_cat_to_num_supervised(spark_session):
    df = read_dataset(spark_session, sample_parquet, "parquet")
    odf= cat_to_num_supervised(spark_session,df, list_of_cols=["workclass","relationship","marital-status"], drop_cols=["ifa"],label_col="income",event_label="<=50K", model_path= "unit_testing/models/")
    assert len(odf.columns) == 17
    ...
    odf= cat_to_num_supervised(spark_session,df, list_of_cols=[], drop_cols=["ifa"],label_col="income",event_label="<=50K")
    assert odf.select("workclass").dtypes[0][1] == "string"
    assert odf.select("marital-status").dtypes[0][1] == "string"
    assert odf.select("relationship").dtypes[0][1] == "string"
    assert odf.select("education").dtypes[0][1] == "string"
