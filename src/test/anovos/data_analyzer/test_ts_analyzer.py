...
def test_ts_analyzer(spark_session):
    ...
    tts_analyzer(
        ...
        id_col="STATE",
        max_days="3600",
        output_path="unit_testing/output/timeseries/ts_analyzer/daily/",
        output_type="daily",
        tz_offset="local",
        run_type="local",
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/daily/" + "YR_EMP_daily.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/daily/" + "YR_HWY_daily.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/daily/" + "YR_P_CAP_daily.csv"
    )
    odf_YR_P_CAP = read_dataset(
        spark_session,
        "unit_testing/output/timeseries/ts_analyzer/daily/YR_P_CAP_daily.csv",
        "csv",
        file_configs={"header": "True", "delimiter": ",", "inferSchema": "True"},
    ...
    tts_analyzer(
        ...
        id_col="STATE",
        max_days="3600",
        output_path="unit_testing/output/timeseries/ts_analyzer/weekly/",
        output_type="weekly",
        tz_offset="local",
        run_type="local",
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/weekly/" + "YR_EMP_weekly.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/weekly/" + "YR_HWY_weekly.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/weekly/" + "YR_P_CAP_weekly.csv"
    )
    odf_YR_HWY = read_dataset(
        spark_session,
        "unit_testing/output/timeseries/ts_analyzer/weekly/YR_HWY_weekly.csv",
        "csv",
        file_configs={"header": "True", "delimiter": ",", "inferSchema": "True"},
    ...
    tts_analyzer(
        ...
        id_col="STATE",
        max_days="1200",
        output_path="unit_testing/output/timeseries/ts_analyzer/hourly/",
        output_type="hourly",
        tz_offset="local",
        run_type="local",
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/hourly/" + "YR_EMP_hourly.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/hourly/" + "YR_HWY_hourly.csv"
    )
    assert os.path.isfile(
        "unit_testing/output/timeseries/ts_analyzer/hourly/" + "YR_P_CAP_hourly.csv"
    )
    odf_YR_HWY = read_dataset(
        spark_session,
        "unit_testing/output/timeseries/ts_analyzer/hourly/YR_HWY_hourly.csv",
