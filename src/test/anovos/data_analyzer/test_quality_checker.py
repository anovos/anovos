...
def test_nullRows_detection(spark_session):
    ...
    result_df = nullRows_detection(spark_session, test_df, treatment=True, treatment_threshold=0.4)
...
def test_outlier_detection(spark_session):
    ...
    result_df7 = outlier_detection(spark_session, test_df7, drop_cols=['ifa', 'label'])[1]
    ...
    assert result_df7.where(F.col("attribute") == "hours-per-week").toPandas().to_dict('list')['upper_outliers'][
               0] == 515
